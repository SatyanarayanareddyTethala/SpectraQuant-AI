"""Equity OHLCV data loader for SpectraQuant-AI-V3.

Loads OHLCV data for canonical equity tickers from the parquet cache
...
Async support
-------------
:meth:`EquityOHLCVLoader.load_many_async` runs symbol loads concurrently using
the :mod:`~spectraquant_v3.core.async_engine` for bounded concurrency and
per-symbol retry.  :meth:`EquityOHLCVLoader.load_many` remains as a
...
def _download_ohlcv_yfinance(
    ...
    Args:
        yf_symbol: yfinance-compatible equity ticker, e.g. ``"INFY.NS"``.
        period:    yfinance period string, e.g. ``"5y"``, ``"1y"``.
...
class EquityOHLCVLoader:
    """Cache-managed OHLCV loader for the equity pipeline.
    ...
    def from_config(
        ...
        Args:
            ...
            registry: Pre-populated :class:`~spectraquant_v3.equities.symbols.registry.EquitySymbolRegistry`.
            run_mode: Cache behaviour.

        Returns:
            A fully-configured :class:`EquityOHLCVLoader`.
        """
        ingestion_cfg = cfg.get("equities", {}).get("ingestion", {})
        return cls(
            cache=cache,
            mapper=EquitySymbolMapper(registry=registry),
            run_mode=run_mode,
            period=ingestion_cfg.get("period", "5y"),
    ...
    def load(self, canonical_symbol: str) -> pd.DataFrame | None:
        """Load OHLCV data for a single canonical equity ticker.
        ...
        Args:
            canonical_symbol: Exchange-qualified equity ticker, e.g. ``"INFY.NS"``.
        ...
        """
        # Resolve to yfinance symbol via mapper; fall back to canonical if not mapped.
        try:
            yf_symbol = self._mapper.to_yfinance_symbol(canonical_symbol)
        except Exception:  # noqa: BLE001
            yf_symbol = canonical_symbol
        ...
        if self._run_mode != RunMode.REFRESH and self._cache.exists(cache_key):
            try:
                ...
                logger.debug(
                    "EquityOHLCVLoader: loaded %s from cache (%d rows)",
                    canonical_symbol,
                    len(df),
                )
                return df
                ...
                logger.warning(
                    "EquityOHLCVLoader: cache read failed for %s: %s; "
        ...
        logger.info(
            "EquityOHLCVLoader: downloading %s (%s) via yfinance …",
            canonical_symbol,
            yf_symbol,
        )
        df = _download_ohlcv_yfinance(yf_symbol, period=self._period, interval=self._interval)
        if df is None or df.empty:
            logger.warning(
                "EquityOHLCVLoader: no OHLCV data returned for %s", canonical_symbol
            )
            return None
        ...
        try:
            ...
            logger.warning(
                "EquityOHLCVLoader: schema validation failed for %s: %s",
                canonical_symbol,
                exc,
            )
            return None
        ...
        try:
            ...
            logger.warning(
                "EquityOHLCVLoader: cache write failed for %s: %s", canonical_symbol, exc
            )
    ...
    def load_many(
        ...
        """Load OHLCV data for multiple canonical equity tickers (sequential).
        ...
        Args:
            canonical_symbols: List of exchange-qualified equity tickers,
                e.g. ``["INFY.NS", "TCS.NS"]``.
    ...
    async def load_many_async(
        ...
        """Load OHLCV data for multiple canonical equity tickers concurrently.
        ...
        for sym, item in summary.results.items():
            if isinstance(item, pd.DataFrame):
                ...
                logger.warning(
                    "EquityOHLCVLoader.load_many_async: '%s' failed – %s",
                    sym,
