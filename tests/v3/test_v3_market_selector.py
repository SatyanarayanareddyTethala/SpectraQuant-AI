...
class TestConfigValidation:
    """Verify that invalid config values raise ValueError at construction time."""

    def test_zero_half_life_raises(self) -> None:
        with pytest.raises(ValueError, match="half_life_hours must be > 0"):
            MarketSelector(config={"half_life_hours": 0})

    def test_negative_half_life_raises(self) -> None:
        with pytest.raises(ValueError, match="half_life_hours must be > 0"):
            MarketSelector(config={"half_life_hours": -1.0})

    def test_negative_top_n_raises(self) -> None:
        with pytest.raises(ValueError, match="top_n must be >= 0"):
            MarketSelector(config={"top_n": -1})

    def test_zero_top_n_is_valid(self) -> None:
        """top_n=0 is allowed — it just produces empty top-records lists."""
        sel = MarketSelector(config={"top_n": 0})
        decision = sel.score([_equity()])
        assert decision.top_equity_records == []

    def test_positive_half_life_and_top_n_are_valid(self) -> None:
        sel = MarketSelector(config={"half_life_hours": 12.0, "top_n": 3})
        assert sel._half_life_hours == 12.0
        assert sel._top_n == 3


# ===========================================================================
# Timestamp normalization ('Z' suffix)
# ===========================================================================


class TestTimestampNormalization:
    """Verify that ISO-8601 timestamps ending in 'Z' are handled correctly."""

    def test_z_suffix_timestamp_does_not_raise(self) -> None:
        """A 'Z'-suffixed timestamp must not fall back to weight 1.0 silently."""
        sel = MarketSelector()
        # Should not raise; recency weight should be computed normally.
        weight = sel._recency_weight("2099-01-01T00:00:00Z")
        assert isinstance(weight, float)
        assert 0.0 < weight <= 1.0

    def test_z_suffix_equals_plus00_suffix(self) -> None:
        """'Z' and '+00:00' for the same instant should give the same weight."""
        sel = MarketSelector()
        w_z = sel._recency_weight("2099-06-15T12:00:00Z")
        w_utc = sel._recency_weight("2099-06-15T12:00:00+00:00")
        assert abs(w_z - w_utc) < 1e-9
